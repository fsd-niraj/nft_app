...
export const ImageCmp = ({imgUrl, i}) => {
  return (
    <Image source={imgUrl} resizeMode="contain" style={{width: 48, height: 48, marginLeft: i === 0 ? 0 : -SIZES.font}} />
  )
}

export const People = () => {

  const users = [assets.person02, assets.person03, assets.person04]

  return (
    <View style={{ flexDirection: "row" }}>
      {users.map((data, i) => 
        <ImageCmp imgUrl={data} key={i} />
      )}
    </View>
  )
}
...
export const EndDate = () => {
  return (
    <View style={{paddingHorizontal: SIZES.font, paddingVertical: SIZES.base, backgroundColor: COLORS.white, justifyContent: "center", alignItems: "center", ...SHADOWS, elevation: 1, maxWidth: "50%"}}>
      <Text style={{fontFamily: FONTS.regular, fontSize: SIZES.small, color: COLORS.primary}}>Endinf in </Text>
